...
// returns 1 if the message has been succesfully copied, 0 if there is not enough space
int ccmd_dump(queue_t* q, message_t* m) {
    ...
    uint32_t avail = m->req_size;

    while (ccmd_itr) {
        m_cmd_itr->cmd = ccmd_itr->cmd;

        if (avail < cmd_type_size(ccmd_itr->cmd))
            return 0;
        avail -= cmd_type_size(ccmd_itr->cmd);
            ...
            case FORWARD_BEGIN:
                if (curr->fwd.proto == DPDK)
                    sprintf(opts, "dpdk://%02x:%02x:%02x:%02x:%02x:%02x,%sb,retries=%d,timeout=%d,branched=%d,nack=%d",
                            curr->fwd.fwd_mac[0], curr->fwd.fwd_mac[1], curr->fwd.fwd_mac[2],
                            curr->fwd.fwd_mac[3], curr->fwd.fwd_mac[4], curr->fwd.fwd_mac[5],
