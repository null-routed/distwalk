...
// match_addr is 6 bytes: either fwd_host(4)+fwd_port(2) or fwd_mac[6],
// same union in fwd_opts_t, so memcmp on fwd_mac covers both cases
...
command_t *single_start_forward(req_info_t *req, message_t *m, command_t *cmd, dw_poll_t *p_poll, conn_worker_info_t *infos) {
    ...
    int fwd_reply_id = find_forward_reply_id(req->curr_cmd, fwd.fwd_mac);
    if (fwd_reply_id == -1)
        return NULL;
    ...
    if (fwd.proto == DPDK) {
        ...
        struct rte_mbuf *tx_mbuf = dpdk_alloc_tx_mbuf(fwd.fwd_mac);
        if (tx_mbuf == NULL) {
...
int find_forward_reply_id(command_t *cmd, const uint8_t match_addr[6]) {
    ...
    for (; cmd != NULL; cmd = cmd_next_forward(cmd), id++) {
        if (memcmp(cmd_get_opts(fwd_opts_t, cmd)->fwd_mac, match_addr, 6) == 0)
            return id;
    }
    return -1;
}
